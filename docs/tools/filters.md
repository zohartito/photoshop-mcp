...
- Filters prefer the ArtLayer DOM `apply*` method (including `shear`, `wave`, `ocean_ripple`, `glass` via `applyGlassEffect`, and `lens_blur` via `applyLensBlur`). Only `box_blur`, `surface_blur`, and `shape_blur` lack a DOM method and use a fixed `executeAction` descriptor. `radial_blur` passes the layer center as its required `blurCenter`. `shape_blur`'s kernel-shape descriptor (`Ellipse 1`) is unverified pending a ScriptingListener capture. All five transforms use the AM `transform` / `warp` event; `distort_corners` / `skew` (2D) / `perspective` use a corner-quad `transform` descriptor whose exact shape is staged for live verification.
